...
export default function SearchPlace() {
  ...
  const detectPlaceFromImage = async (base64Image) => {
    const openAiApiKey = getOpenAiApiKey();
    if (!openAiApiKey || openAiApiKey.includes("YOUR_")) {
      throw new Error(
        "Fotoğraftan yer bulmak için EXPO_PUBLIC_OPENAI_API_KEY gerekli.",
      );
    }
        ...
        messages: [
          {
            role: "system",
            content:
              "Görüntüden mekan/yeri tahmin eden bir asistansın. Sadece geçerli JSON dön.",
          },
          {
            role: "user",
            content: [
              {
                type: "text",
                text: "Bu fotoğrafın neresi olabileceğini tahmin et. Eğer belirgin bir yer veya bölge görüyorsan dön. Sadece şu JSON formatında yanıt ver: {\"placeName\":\"...\",\"city\":\"...\",\"country\":\"...\",\"confidence\":0-1,\"reason\":\"kısa açıklama\"}. Emin değilsen placeName alanını boş bırak.",
              },
              {
                type: "image_url",
                image_url: {
    ...
    if (!response.ok) {
      ...
      const errorMessage =
        ...
        `OpenAI hatası: ${response.status} ${response.statusText}`;
      throw new Error(errorMessage);
    }
    ...
    try {
      ...
      throw new Error("Fotoğraf analizi yanıtı çözümlenemedi.");
    }

    return parsed;
  };

  const searchPlaceFromText = async (query) => {
    const apiKey = getGooglePlacesApiKey();
    if (!apiKey || apiKey.includes("YOUR_")) {
      throw new Error("Google Places API key bulunamadı.");
    }
    ...
    if (!response.ok) {
      ...
      const errorMessage =
        errorData?.error?.message ||
        `Google Places hatası: ${response.status} ${response.statusText}`;
      throw new Error(errorMessage);
    }
  ...
  const handleImagePlaceDetect = async () => {
    try {
      const permission =
      ...
      const aiResult = await detectPlaceFromImage(result.assets[0].base64);
      const query = [aiResult?.placeName, aiResult?.city, aiResult?.country]
        .filter(Boolean)
        .join(", ");
      ...
      if (!query) {
        Alert.alert(
          "Yer Bulunamadı",
          "Fotoğraftan net bir konum çıkarılamadı. Lütfen farklı bir fotoğraf deneyin.",
        );
        return;
      }

      const placeId = await searchPlaceFromText(query);
      if (!placeId) {
        Alert.alert(
          "Yer Eşleşmedi",
          "Tahmin edilen yer Google Places üzerinde bulunamadı.",
        );
        return;
      }

      await fetchPlaceDetails(placeId, null, null);
      setSearchText(query);
      setPredictions([]);

      Alert.alert(
        "Yer Bulundu",
        `${query} tahmin edildi. Kontrol edip devam edebilirsiniz.`,
      );
    } catch (error) {
      Alert.alert(
        "Fotoğraf Analiz Hatası",
        error?.message || "Fotoğraf işlenirken bir sorun oluştu.",
      );
    } finally {
  ...
  return (
    ...
    >
      ...
      <TouchableOpacity
        onPress={handleImagePlaceDetect}
