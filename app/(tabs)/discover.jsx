...
const defaultPlaces = [
  {
    ...
    image: 'https://images.unsplash.com/photo-1502602898657-3e91760cbb34?auto=format&fit=crop&w=900&q=60',
  },
  {
    id: 'nyc',
    title: 'New York, ABD',
    description: 'Times Square • Central Park • Broadway',
    image: 'https://images.unsplash.com/photo-1505764706515-aa95265c5abc?auto=format&fit=crop&w=900&q=60',
  },
  {
    id: 'istanbul',
...
export default function Discover() {
  ...
  const fetchPlaces = async (text, retryCount = 0) => {
    ...
    try {
      ...
      const response = await fetch(
        ...
        {
          headers: {
            'User-Agent': 'YolArkadasim-TravelApp/1.0 (https://yolarkadasim.app)', // Nominatim için gerekli - daha detaylı
      ...
      if (Array.isArray(data) && data.length > 0) {
        const mapped = data.map((place, index) => {
          ...
          const placeName = place.display_name?.split(',')[0] || place.name || 'travel';
          const imageUrl = getPlaceImageUrl(placeName);
          
          return {
            id: place.place_id || place.osm_id || `place-${index}`,
            placeId: place.place_id || place.osm_id,
            title: place.display_name?.split(',')[0] || place.name || 'Bilinmeyen Yer',
            description: place.display_name || place.address?.road || 'Haritada açmak için dokunun',
            coordinates: {
      ...
      if (error.name === 'AbortError') {
        ...
        console.warn('⚠️ Nominatim API yavaş yanıt veriyor. Varsayılan yerler gösteriliyor.');
        showDefaultIfEmpty();
        return;
      }
      ...
      if (error.message?.includes('Network request failed') || error.message?.includes('Failed to fetch')) {
        ...
        console.warn('⚠️ İnternet bağlantısı hatası. Varsayılan yerler gösteriliyor.');
        showDefaultIfEmpty();
        return;
      } else {
  ...
  const handleSavePlace = async (place, e) => {
    ...
    if (!auth.currentUser) {
      Alert.alert('Giriş Gerekli', 'Yerleri kaydetmek için lütfen giriş yapın.');
      return;
    }
    ...
    if (!placeId || placeId === 'unknown') {
      Alert.alert('Hata', 'Bu yer kaydedilemiyor. Lütfen başka bir yer deneyin.');
      return;
    }
    ...
    try {
      if (isSaved) {
        ...
        Alert.alert('Bilgi', 'Yeri kaldırmak için Kaydedilen Yerler sayfasına gidin.');
      } else {
        // Yeri kaydet
        ...
        const safePlaceId = String(placeId || 'unknown').replace(/[^a-zA-Z0-9._-]/g, '_');
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <View style={{ flex: 1 }}>
                <Text style={{ fontFamily: 'outfit-bold', fontSize: 18, color: Colors.WHITE }}>
                  {item.title}
                </Text>
                {item.rating && (
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4 }}>
                    <Ionicons name="star" size={16} color="#F6B100" />
                    <Text style={{ color: Colors.WHITE, marginLeft: 6 }}>{item.rating.toFixed(1)}</Text>
                  </View>
                )}
              </View>
              {auth.currentUser && (
                <TouchableOpacity
                  ...
                  style={{
                    padding: 8,
                    borderRadius: 20,
                    backgroundColor: saved ? 'rgba(239, 68, 68, 0.9)' : 'rgba(255, 255, 255, 0.3)',
                  }}
                >
                  <Ionicons 
        ...
        <View style={{ padding: 16 }}>
          <Text style={{ fontFamily: 'outfit', color: Colors.GRAY }}>{item.description}</Text>
          <Text
            style={{
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F5F7FB', paddingHorizontal: 25, paddingTop: 60 }}>
      <Text style={{ fontFamily: 'outfit-bold', fontSize: 32, color: Colors.PRIMARY }}>
        Keşfet
      </Text>
      <Text style={{ fontFamily: 'outfit', color: Colors.GRAY, marginTop: 6, marginBottom: 20 }}>
