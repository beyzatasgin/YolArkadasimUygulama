...
const quickActions = [
  {
    icon: "airplane",
    title: "Yaklaşan Seyahatler",
    subtitle: "Planlanan yolculuklarınızı görün",
    target: "/mytrip",
  },
  {
    icon: "heart",
...
export default function Profile() {
  ...
  const uploadPhotoToStorage = async (uri) => {
    ...
    try {
      ...
      const photoRef = ref(
        storage,
        `profile-photos/${auth?.currentUser?.uid}/${timestamp}.jpg`,
      );
      ...
      const errorMessage =
        error.message || "Fotoğraf yüklenirken bilinmeyen bir hata oluştu";
    ...
    try {
      ...
      if (hasNewPhoto) {
        if (!storage) {
          Alert.alert(
            "Hata",
            "Firebase Storage başlatılamadı. Fotoğraf yüklenemedi, ancak isim güncellenebilir.",
          );
      ...
      if (db && auth?.currentUser) {
        await setDoc(
          doc(db, "userProfiles", auth.currentUser.uid),
          {
            ...
            searchableEmail: (auth.currentUser.email || "").toLowerCase(),
  ...
  const handlePickImage = async () => {
    ...
    try {
      ...
      if (!permissionResult.granted) {
        Alert.alert(
          "İzin Gerekli",
          "Lütfen fotoğraf kütüphanenize erişim izni verin.",
        );
        return;
      }
    ...
    <ScrollView style={{ flex: 1, backgroundColor: Colors.WHITE }}>
      <View style={{ padding: 25, paddingTop: 60 }}>
        ...
        >
          ...
          <Text
            style={{ fontFamily: "outfit-bold", fontSize: 20, marginTop: 8 }}
          >
            {editing
              ? "Adınızı güncelleyin"
          ...
          {editing ? (
            <View style={{ width: "100%", gap: 12, marginTop: 8 }}>
              ...
              <View style={{ flexDirection: "row", gap: 10, marginTop: 8 }}>
                ...
                >
                  <Text
                    style={{ color: Colors.GRAY, fontFamily: "outfit-medium" }}
                  >
                    İptal
                  </Text>
                </TouchableOpacity>
        ...
        >
          ...
          <Text style={{ fontFamily: "outfit-medium", color: "#B91C1C" }}>
            Çıkış Yap
          </Text>
        </TouchableOpacity>
      </View>
