...
export default function Mytrip() {
  ...
  const handleShareTripWithFriends = async (trip) => {
    if (!auth?.currentUser || !db) {
      Alert.alert("Hata", "Rota paylaşmak için giriş yapmanız gerekiyor.");
      return;
    }
    ...
    try {
      ...
      const friendshipSnapshot = await getDocs(friendsQuery);
      const friendUids = friendshipSnapshot.docs
        .map((snap) => snap.data()?.users || [])
        .flat()
        .filter((uid) => uid && uid !== auth.currentUser.uid);

      if (friendUids.length === 0) {
        Alert.alert("Arkadaş Bulunamadı", "Önce Sosyal sekmesinden arkadaş ekleyin.");
        return;
      }

      const shareWrites = friendUids.map((friendUid) =>
        setDoc(doc(db, "tripShares", `${trip.id}_${friendUid}`), {
          tripId: trip.id,
          tripName: trip.tripName || "Seyahat Rotası",
          placeName: trip.selectedPlace?.name || "",
          placeAddress: trip.selectedPlace?.address || "",
          placePhotoUrl: trip.selectedPlace?.photoUrl || null,
          toUserId: friendUid,
          fromUserId: auth.currentUser.uid,
          fromUserName: auth.currentUser.displayName || "Kullanıcı",
      ...
      Alert.alert(
        "Paylaşıldı",
        `Rota ${friendUids.length} arkadaşınla paylaşıldı.`,
      );
    } catch (error) {
      Alert.alert("Hata", error?.message || "Rota paylaşılırken hata oluştu.");
    }
  };
        ...
        >
          ...
          <View style={{ padding: 20 }}>
            ...
            <TouchableOpacity
              ...
              onPress={(e) => {
                e.stopPropagation();
                handleShareTripWithFriends(item);
              }}
            >
              <Text
                style={{
