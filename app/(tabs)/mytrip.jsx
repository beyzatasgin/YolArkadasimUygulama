...
export default function Mytrip() {
  ...
  useEffect(() => {
    ...
    console.log('🔄 Setting up trips listener for user:', auth.currentUser.uid);
      ...
      unsubscribe = onSnapshot(
        ...
        (error) => {
          ...
          if (error.code === 'failed-precondition') {
            console.warn('⚠️ Firestore composite index gerekiyor. Firebase Console\'da index oluşturun.');
  ...
  }, [auth.currentUser?.uid]); // auth.currentUser.uid değiştiğinde yeniden başlat
  ...
  // İlgi alanı ID'sini label'a çevir
  ...
  // Trip durumunu kontrol et (yaklaşan/geçmiş)
  const getTripStatus = (startDate) => {
    ...
    tripStart.setHours(0, 0, 0, 0);
    return tripStart >= today ? 'upcoming' : 'past';
  };
  ...
  const renderTripCard = ({ item }) => {
    const tripStatus = getTripStatus(item.startDate);
    ...
    const placeImageUrl = item.selectedPlace?.photoUrl || getPlaceImageUrl(item.selectedPlace?.name);
    ...
    >
      ...
      <TouchableOpacity
        ...
        onPress={(e) => {
          e.stopPropagation();
          handleDeleteTrip(item.id, item.tripName || item.selectedPlace?.name || 'Seyahat');
        }}
      >
      ...
      <View style={{ padding: 20 }}>
        {/* Yer Adı */}
        <Text style={{ fontFamily: 'outfit-bold', fontSize: 18, color: Colors.PRIMARY, marginBottom: 8 }}>
          {item.selectedPlace?.name || 'Bilinmeyen Yer'}
        </Text>
        
        {/* Tarih */}
        {item.startDate && (
          <Text style={{ fontFamily: 'outfit', fontSize: 14, color: Colors.GRAY, marginBottom: 12 }}>
        ...
        {item.travelers && (
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 15 }}>
            ...
            <Text style={{ fontFamily: 'outfit-medium', fontSize: 14, color: Colors.PRIMARY, marginLeft: 6 }}>
              {item.travelers === 1 ? 'Yalnız' : item.travelers === 2 ? 'Arkadaşlar' : `${item.travelers} Kişi`}
            </Text>
          </View>
        )}
        ...
        >
          <Text style={{ fontFamily: 'outfit-medium', fontSize: 16, color: Colors.WHITE }}>
            Planını Gör
          </Text>
        </TouchableOpacity>
      </View>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.WHITE }}>
      ...
      >
        <Text style={{ fontFamily: 'outfit-bold', fontSize: 35, color: Colors.PRIMARY }}>
          Seyahatlerim
        </Text>
      ...
      {loading ? (
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
          ...
          <Text style={{ fontFamily: 'outfit', color: Colors.GRAY, marginTop: 10 }}>
        ...
        >
          ...
          {upcomingTrips.length > 0 && (() => {
            ...
            const today = new Date();
            today.setHours(0, 0, 0, 0);
            const tripStart = nextTrip.startDate ? new Date(nextTrip.startDate) : today;
            tripStart.setHours(0, 0, 0, 0);
