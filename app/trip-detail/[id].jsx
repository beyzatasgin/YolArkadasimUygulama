...
export default function TripDetailScreen() {
  ...
  const getFallbackImageUrl = (placeName) => {
    try {
      if (!placeName) return 'https://images.unsplash.com/photo-1488646953014-85cb44e25828?auto=format&fit=crop&w=900&q=80';
  ...
  const openInMaps = () => {
    if (trip?.selectedPlace?.coordinates) {
      ...
      const url = trip.selectedPlace.url || `https://www.openstreetmap.org/?mlat=${lat}&mlon=${lon}&zoom=15`;
  ...
  const finalImageUrl = placeImageUrl || trip?.selectedPlace?.photoUrl || getFallbackImageUrl(trip?.selectedPlace?.name);
    ...
    <ScrollView style={styles.container}>
      <View style={styles.content}>
        ...
        <View style={[styles.headerSection, { paddingHorizontal: 25 }]}>
          ...
          {tripStatus && (
            <View
              style={[
                styles.statusBadge,
                tripStatus === 'Yaklaşan' ? styles.statusBadgeUpcoming : styles.statusBadgePast,
              ]}
            >
              <Text
                style={[
                  styles.statusBadgeText,
                  tripStatus === 'Yaklaşan' ? styles.statusBadgeTextUpcoming : styles.statusBadgeTextPast,
                ]}
              >
                {tripStatus}
              </Text>
        ...
        <View style={[styles.card, { marginHorizontal: 25 }]}>
          ...
          <Text style={styles.cardValue}>{trip.selectedPlace?.name || 'Belirtilmemiş'}</Text>
          {trip.selectedPlace?.address && (
            <Text style={styles.cardSubValue}>{trip.selectedPlace.address}</Text>
          )}
            ...
            <View style={styles.interestsContainer}>
              {trip.interests.map((interest, index) => (
                <View key={index} style={styles.interestTag}>
                  <Text style={styles.interestTagText}>{getInterestLabel(interest)}</Text>
                </View>
              ))}
            </View>
          </View>
          ...
          <View style={[styles.card, { marginHorizontal: 25 }]}>
                    ...
                    onPress={() => {
                      router.push({
                        ...
                        params: {
                          ...
                          placeCoordinates: JSON.stringify(trip.selectedPlace?.coordinates || {}),
                          recommendations: JSON.stringify(trip.aiPlan.recommendations || {}),
                        },
                      });
                    }}
                  >
            ...
            {trip.aiPlan.recommendations && (
              <View style={styles.recommendationsSection}>
                {trip.aiPlan.recommendations.accommodations && trip.aiPlan.recommendations.accommodations.length > 0 && (
                  <View style={styles.recommendationGroup}>
                    <Text style={styles.recommendationTitle}>🏨 Konaklama Önerileri</Text>
                    {trip.aiPlan.recommendations.accommodations.map((acc, index) => {
                      ...
                      return (
                        <View key={index} style={{ marginBottom: 4 }}>
                          <Text style={styles.recommendationItem}>• {accName}</Text>
                          {accRating && (
                            <Text style={[styles.recommendationItem, { fontSize: 12, marginLeft: 10, color: Colors.GRAY }]}>
                ...
                {trip.aiPlan.recommendations.restaurants && trip.aiPlan.recommendations.restaurants.length > 0 && (
                  <View style={styles.recommendationGroup}>
                    <Text style={styles.recommendationTitle}>🍽️ Restoranlar</Text>
                    {trip.aiPlan.recommendations.restaurants.map((restaurant, index) => (
                      <Text key={index} style={styles.recommendationItem}>• {restaurant}</Text>
                    ))}
                  </View>
                )}

                {trip.aiPlan.recommendations.attractions && trip.aiPlan.recommendations.attractions.length > 0 && (
                  <View style={styles.recommendationGroup}>
                    <Text style={styles.recommendationTitle}>📍 Görülecek Yerler</Text>
                    {trip.aiPlan.recommendations.attractions.map((attraction, index) => (
                      <Text key={index} style={styles.recommendationItem}>• {attraction}</Text>
                    ))}
                  </View>
                )}

                {trip.aiPlan.recommendations.tips && trip.aiPlan.recommendations.tips.length > 0 && (
                  <View style={styles.recommendationGroup}>
                    <Text style={styles.recommendationTitle}>💡 İpuçları</Text>
                    {trip.aiPlan.recommendations.tips.map((tip, index) => (
                      <Text key={index} style={styles.recommendationItem}>• {tip}</Text>
                    ))}
                  </View>
                )}
              </View>
